    ...
    <div class="container">
        <div id="content">
            <div class="img" style="width: 120px">
                {{-- <img src="{{ asset('wk.png') }}" alt="" width="120"> --}}
            </div>
            <div class="cont-info">
        ...
        <div class="tanggal">
            <p>
                ...
                @endphp
                {{ Carbon\Carbon::parse($surat['created_at'])->formatLocalized('%d %B %Y') }}
            </p>
        </div>
        <div class="head-let">
            <div class="about-let">
                No: @if($surat->first()->letter_type_id)
                {{ $surat->first()->letterType->letter_code }}/SMK Wikrama/{{ $surat->first()->created_at->format('Y') }}
            @else
                No notulis assigned
            @endif<br>
                Hal: <b>{{ $surat->letter_perihal }}</b>
            </div>
            <div class="for-let">
        ...
        <div class="main-content">
            ...
            <div class="peserta">
                Peserta: 
                <ol>
                    @foreach ($surat['recipients'] as $item)
                    <li>{{ $item['name'] }}</li>
                    @endforeach
                </ol>
